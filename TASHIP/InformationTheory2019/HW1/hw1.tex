...
\part[10] Using the entropy concept, calculate the expected codeword length for
  each set.
  \begin{solution}
      One case is trivial that all 8 horses belongs to
      probability class \(2^{-3}\). Each horse has the probability of winning
      \(\frac{1}{8}\).
  \end{solution}
\part[10] For each set, present an efficient binary coding scheme that achieves
  optimality.
  \begin{solution}
